...
async function performDeepResearch(theme: string) {
  ...
  const researchPrompt = `
    ...
    各主張には必ず対応する論文PMIDを角括弧で付記すること（例: 「ビタミンCは光老化を改善する[PMID: 12345678]」）。

    # 参照論文リスト
    記事本文で引用する価値のある論文を3〜6件、以下のフォーマットで列挙すること。
    必ず実在する論文のみ（PubMedで検索可能な論文のみ）。不明な場合は「該当なし」と明記し、架空の論文を捏造しないこと。
...
async function generateBlogPost(theme: string, researchData: string, imageUrl: string, publishDate: string) {
  ...
  const writingPrompt = `
    ...
    ## 最後に、心を込めて。
    ## 参考文献
    （【厳守】リサーチデータの「# 参照論文リスト」セクションに列挙された論文から、記事本文で実際に根拠として用いたものを2〜4件、以下のMarkdownフォーマットで必ず列挙すること：
    - 著者名. 論文タイトル. *雑誌名*. 年;巻(号):ページ. PMID: xxxxxxxx. [DOI: xxxx](https://doi.org/xxxx)

    曖昧な「信頼できる情報源に基づき〜」は絶対に禁止。
    リサーチデータにPMIDが含まれていない場合でも、著者・雑誌・年は必ず記載すること。
    該当論文が一つも無い場合は「参考文献」セクション自体を省略し、「本記事は一般的な医学情報の範囲でまとめています」と1行記載すること。

    最後に以下の1行を必ず付け加えること（論文がある場合のみ）：
