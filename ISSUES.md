...
### x-patrol 自動リプライ 403 エラー
- **初検知:** 2026-04-24
- **原因:** Twitter Free Tier でリプライ API が 403（権限不足）を返す
- **対応:** 自動リプライをやめ、Slack提案モード（ワンタップ返信ボタン付き）に変更

### ブログ・Xスレッドのテーマ重複
- **初検知:** 2026-04-28（取締役指摘）
- **原因:** plan-10day-themes.ts に隣接日の類似チェックがなく、ThemeArea がクラスタリングされていた
- **対応:** ①→②→③→④→⑤ ローテーション指示 + 同カテゴリ素材の連日禁止を追加。X スレッドの文体バリエーション5パターンも追加
